...
creative_model = init_chat_model(
    model=CREATIVE_MODEL, api_key=OPENAI_API_KEY, max_retries=3
)

# Writer model for final report
writer_model = init_chat_model(
    model=WRITER_MODEL, api_key=OPENAI_API_KEY, max_retries=3
)
...
compressor_model = init_chat_model(
    model=COMPRESSOR_MODEL, api_key=OPENAI_API_KEY, max_retries=3
)
...
critic_model = init_chat_model(
    model=CRITIC_MODEL, api_key=OPENAI_API_KEY, max_retries=3
)
